...
public:
  ...
  void spawnZombieAt (int row_y, int x = WINDOW_WIDTH - 1,
                      ZombieType type = ZombieType::ZOMBIE_REGULAR);
  ...
  bool
  anyZombieOnRow (int row)
  {
    return m_row_zombie_cnt[row] > 0;
  }
...
private:
  ...
  std::list<std::list<std::shared_ptr<GameObject> >::iterator>
      m_zombie_it_list;
