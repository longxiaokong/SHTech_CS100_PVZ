...
Repeater::Repeater (pGameWorld pWorld, int x, int y)
    : Plant (pWorld, PlantType::PLANT_REPEATER, x, y),
      m_shoot_timer (SHOOTER_INTERVAL), m_bullet_timer (0)
{
  m_shoot_timer.StartTimer ();
}
  ...
  if (m_bullet_timer.isTiming () && m_bullet_timer.isTimingEnded ())
    {
      ...
      getGameWorld ()->spawnProjectileAt (GetX () + 30, GetY () + 20,
                                          ProjectileType::PROJ_PEA);
    }
  if (m_shoot_timer.isTimingEnded ()
      && getGameWorld ()->anyZombieOnRow (GetGridY ()))
    {
