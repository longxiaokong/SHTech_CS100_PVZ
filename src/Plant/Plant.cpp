...
Plant::Plant (pGameWorld pWorld, PlantType type, int x, int y)
    : GameObject (pWorld, plantImageID[static_cast<std::size_t> (type)],
                  Plant::gridToPixelX (x), Plant::gridToPixelY (y), LAYER_UI,
                  LAWN_GRID_WIDTH, LAWN_GRID_HEIGHT, ANIMID_IDLE_ANIM),
      m_type (type), m_health (plantHealth[static_cast<std::size_t> (type)])
{
}
...
Plant::~Plant ()
{
  if (m_type != PlantType::PLANT_NONE)
    getGameWorld ()->PlantAtPos (GetGridX (), GetGridY (),
